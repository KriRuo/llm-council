...
@limiter.limit("20/minute")  # Limit conversation creation
...
@limiter.limit("10/minute")  # Limit message sending to prevent API abuse
async def send_message(request: Request, conversation_id: str, req: SendMessageRequest):
    """
...
@limiter.limit("10/minute")  # Limit streaming endpoint
async def send_message_stream(request: Request, conversation_id: str, req: SendMessageRequest):
    """
    ...
    async def event_generator():
        try:
            ...
            # Send error event (don't expose detailed error info)
            logger.error(f"Error in streaming endpoint: {type(e).__name__}", exc_info=True)
...
if __name__ == "__main__":
    import uvicorn
    # Bind to localhost only for security (not accessible from network)
